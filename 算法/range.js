...
class RangeList {
  ...
  add(range) {
    ...
    if (minIndex === maxIndex) {
      if (min > list[minIndex][1]) {
        ...
        list[minIndex] = [Math.min(min, list[minIndex][0]), Math.max(max, list[minIndex][1])]
      }
    }
    // change
    if (maxIndex > minIndex) {
      if (min > list[minIndex][1]) {
        list[maxIndex] = [Math.min(min, list[maxIndex][0]), Math.max(max, list[maxIndex][1])]
      } else {
        list.splice(
          minIndex,
          (maxIndex - minIndex + 1), [Math.min(min, list[minIndex][0]),
  ...
  remove(range) {
    ...
    if (maxIndex > minIndex) {
      if (min >= list[minIndex][0]) {
        max < list[maxIndex][1] ?
          list.splice(minIndex, maxIndex - minIndex + 1, [list[minIndex][0], min], [max, list[maxIndex][1]]) :
          list.splice(minIndex, maxIndex - minIndex + 1, [list[minIndex][0], min])
      } else {
        max < list[maxIndex][1] ?
          list.splice(minIndex, maxIndex - minIndex + 1, [min, list[minIndex][0]], [max, list[maxIndex][1]]) :
          list.splice(minIndex, maxIndex - minIndex + 1, [min, list[minIndex][0]])
      }
    }
  }
   ...
   */
  print() {
    const pri = this.list.reduce((acc, item) => {
